...
namespace lb01
{
    ...
    {
        public string PID { get; set; }//点名
        public double X { get; set; }//X坐标
        public double Y { get; set; }//Y坐标
        public bool IsCommonP { get; set; } //是否是公共点
    }
    ...
    public class Matrix
    {
        ...
        //矩阵相加实现
        public static Matrix subtraction(Matrix Lm, Matrix Rm)
        {
            if (Lm.row != Rm.row)
            {
        ...
        public static Matrix Mutiply(Matrix m1, Matrix m2)
        {
            ...
            if (m1.col != m2.row)
            {
                System.Exception e = new Exception("前矩阵列数不等于后矩阵行数,无法相乘");
                throw e;
            }
        ...
        public static Matrix Inverse(Matrix M)
        {
            ...
            if (m != n)
            {
                System.Exception myException = new Exception("求逆的矩阵不是方阵，不能求逆");
                throw myException;
            }
